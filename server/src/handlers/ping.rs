...
use crate::{config::Config, services::api::perform_ping, state::AppState};

#[utoipa::path(
    get,
    path = "/api/ping/{node_name}",
    tag = "tools",
    params(
        ("node_name" = String, Path, description = "Node name"),
        ("request_id" = String, Query, description = "Client-supplied request identifier"),
        ("target" = String, Query, description = "Ping target"),
        ("version" = Option<String>, Query, description = "Optional IP version override")
    ),
    responses(
        (
            status = 200,
            description = "Server-Sent Events stream. Each event data field is a JSON-serialized AppResponse ping message.",
            content_type = "text/event-stream",
            body = String
        )
    )
)]
pub async fn proxy_ping(
