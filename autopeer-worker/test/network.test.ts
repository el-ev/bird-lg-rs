...
`;
  ...
  it("treats empty string psk as clear (removes existing PSK)", async () => {
    const fileWithPsk = `peers:
  - comment: 'autopeer test'
    wg:
      port: 21234
      endpoint: 'peer.example.net:21023'
...
`;
    ...
    const result = await mutatePeerFile(fileWithPsk, {
      asn: "4242421234",
      effectiveMnt: "EXAMPLE-MNT",
      authMethod,
      kind: "update",
    ...
    expect(result.content).not.toContain("psk: !vault");
    expect(result.content).toContain("psk: null");
  });
